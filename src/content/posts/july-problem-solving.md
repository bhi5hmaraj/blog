...
$$ DP_k[arr[i]] = \sum_{\substack{j > i \\ arr[j] < arr[i]}} DP_{k-1}[arr[j]] $$

The base case is $DP_1[val] = 1$ for all $1 \leq val \leq n$, this is because there is only $1$ subsequence of length $1$ starting with $val$
